...
//       console.log(taskerProfileCheck, "tasker profile ")
...
//           console.log(errorData.missingFields, "error data")
...
export default function TaskListSection() {
  ...
  const hasBothRoles = user?.roles?.includes("client") && user?.roles?.includes("tasker");
  const currentRole = user?.currentRole || "client";
  const taskerStatus = user?.taskerStatus; // Get taskerStatus from user data
  ...
  const switchRole = async (newRole: "tasker" | "client") => {
    ...
    // Check if switching to tasker and taskerStatus is not approved
    if (newRole === "tasker" && taskerStatus !== "approved") {
      toast.info("Complete your Tasker profile first to unlock this mode.");
      console.log(taskerStatus, "tasker status not approved");
      router.push("/complete-tasker-profile");
      return;
    }
  ...
  return (
    <section className="min-h-screen bg-gradient-to-b from-gray-50 to-white">
      ...
      {(hasBothRoles || (user && !user.roles?.includes("tasker"))) && (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6">
          <div className={`border rounded-2xl p-4 sm:p-5 ${hasBothRoles && taskerStatus === "approved"
            ? "bg-gradient-to-r from-[#E5FFDB] to-[#d4f5c7] border-[#109C3D]/20"
            : "bg-gradient-to-r from-blue-50 to-indigo-50 border-blue-200"
            }`}>
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                <div className={`w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0 ${hasBothRoles && taskerStatus === "approved" ? "bg-[#109C3D]" : "bg-blue-500"
                  }`}>
                  <FaExchangeAlt className="text-white text-xl" />
                </div>
                <div>
                  {hasBothRoles && taskerStatus === "approved" ? (
                    <>
                      <h3 className="font-semibold text-[#063A41]">
                        You're in <span className="text-[#109C3D]">Booker</span> Mode
                      </h3>
                    ...
                    <>
                      ...
                      <p className="text-sm text-gray-600 mt-0.5">
                        {taskerStatus === "pending"
                          ? "Your Tasker profile is under review"
                          : taskerStatus === "rejected"
                            ? "Your Tasker application needs attention"
                            : "Become a Tasker and start offering your services"}
                      </p>
                    </>
                  )}
